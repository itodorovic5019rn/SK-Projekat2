package com.raf.notificationservice.security;

import com.google.gson.Gson;
import com.raf.notificationservice.exception.Message;
import com.raf.notificationservice.security.service.TokenService;
...
public class SecurityAspect {
    ...
    @Around("@annotation(com.raf.notificationservice.security.CheckSecurity)")
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (token == null) {
            Message msg = new Message("Token not provided");
            return new ResponseEntity<>(gson.toJson(msg), HttpStatus.UNAUTHORIZED);
        }
        //Try to parse token
        Claims claims = tokenService.parseToken(token);
        //If fails return UNAUTHORIZED response
        if (claims == null) {
            return new ResponseEntity<>(gson.toJson(new Message("Invalid token!")), HttpStatus.UNAUTHORIZED);
        }
        ...
        return new ResponseEntity<>(gson.toJson(new Message("Not authorized!")), HttpStatus.FORBIDDEN);
    }
